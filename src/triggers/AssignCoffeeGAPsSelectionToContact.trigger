trigger AssignCoffeeGAPsSelectionToContact on Coffee_Farmer_GAPs_Selection__c (after insert) {
  List<Coffee_Farmer_GAPs_Selection__c> coffeeFarmerGAPs = trigger.new;
    ...
    for (Coffee_Farmer_GAPs_Selection__c coffeeFarmerGAP : coffeeFarmerGAPs) {
        contactIds.add(coffeeFarmerGAP.CKW_Contact__c);
        
        // Check whether the contact for the current farmer already has an entry
        // in the farmerGapsByContact map
        List<String> contactFarmers = farmerGapsByContact.get(coffeeFarmerGAP.CKW_Contact__c);
        if (contactFarmers == null) {
            contactFarmers = new List<String>();
        }
        contactFarmers.add(coffeeFarmerGAP.Id);
        farmerGapsByContact.put(coffeeFarmerGAP.CKW_Contact__c, contactFarmers);
    }
    ...
    List<gfsurveys__SObjectContactAssociation__c> socas =  [Select Id, gfsurveys__AssociatedIds__c, 
                                                        ...
                                                        WHERE gfsurveys__SObjectApiName__c = 'Coffee_Farmer_GAPs_Selection__c'
    ...
    // Iterate thru the farmerGapsByContact map, to create or update a soca with the coffee farmer gaps ids for each contact
    for (String contactId : farmerGapsByContact.keyset()) {
        List<String> coffeeFarmerGAPsIds = farmerGapsByContact.get(contactId);
        ...
        if (soca == null) {
            ...
            soca.gfsurveys__SObjectApiName__c = 'Coffee_Farmer_GAPs_Selection__c';
            soca.gfsurveys__AssociatedIds__c = String.join(coffeeFarmerGAPsIds, ',');
            soca.gfsurveys__Contact__c = contactId;
            soca.gfsurveys__NumberOfRecords__c = coffeeFarmerGAPsIds.size();
        }
        else {
            soca.gfsurveys__AssociatedIds__c = soca.gfsurveys__AssociatedIds__c + ',' + String.join(coffeeFarmerGAPsIds, ',');
            soca.gfsurveys__NumberOfRecords__c += coffeeFarmerGAPsIds.size();
        }
