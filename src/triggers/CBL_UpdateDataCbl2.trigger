trigger CBL_UpdateDataCbl2 on CBL_CoffeeBaseline__c (after insert) {
	List<CBL_CoffeeBaseline__c> cbls = new List<CBL_CoffeeBaseline__c>();
    ...
    List<CBL2_Farmer__c> ExistingFarmers = [SELECT Id, LAC_Farmer_Details__c from CBL2_Farmer__c WHERE LAC_Farmer_Details__c IN : LAC_FD_Ids];
    for(CBL_CoffeeBaseline__c cbl : cbls){
        farmerExists = false;
