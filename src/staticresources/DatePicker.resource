...
//******************************************************************************
    ...
    // Add the timestamp on the end. if this var is true then the time added is 23:59:59 or it is 00:00:00
    var isEndDate = false;
...
// ****************************************************************************
    ...
    function scwShow(scwEle,scwSource)
        {if (!scwSource) {scwSource = window.event;}
         ...
         if (scwDateValue.length==0 || scwDateValue == "Enter Date")
         ...
         if (isNaN(scwSeedDate))
            {if (scwShowInvalidDateMsg && scwSeedDate != "Enter Date") {alert(scwInvalidDateMsg + scwInvalidAlert[0] + scwDateValue + scwInvalidAlert[1]);}
             scwSeedDate = new Date(scwBaseYear + Math.floor(scwDropDownYears/2),5,1);
             scwBlnFullInputDate=false;
            }
         else
    ...
    function scwShowMonth(scwBias)
             ...
             function scwSetOutput(scwOutputDate)
                {
                 ...
                 else {scwTargetEle.value = scwOutputDate.scwFormat(scwDateOutputFormat) + timeString;}
                 scwHide();
                };
        ...
        // Change the time string controlling variable.
        function changeTimeString(bool)
        {
        	 if (bool == 1)
           	 {
           		 isEndDate = true;
